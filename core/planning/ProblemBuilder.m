classdef ProblemBuilder < handle
    ...
    methods
        function obj = ProblemBuilder(agents, env_params, current_params, sim_params, agent_params, config)
            ...
            obj.formation_constraint_indices = [];
            obj.formation_constraint_violations = [];
        ...
        function buildBenchmarkingConstraintsAndBounds(obj)
            ...
            if obj.config.enable_obstacle_constraints && ~isempty(obj.env_params.obstacles)
                n_additional = n_additional + obj.T * obj.N_agents * length(obj.env_params.obstacles);
            end
            ...
            for k = 1:obj.T
                ...
                for i = 1:obj.N_agents
                    if ~bench
                        ...
                        Agent_pos_ik_P0 = P0_k_plus_1_matrix(:, i); 
                        for j = 1:n_obs
                            dist_sq_all_obs_P0 = sumsqr(Agent_pos_ik_P0 - obs_centers_matrix(:, j));
            ...
            for k = 1:obj.T
                ...
                for i = 1:obj.N_agents
                    ...
                    for j = 1:n_obs
                        ...
                        dist_sq_linearized = sumsqr(Agent_pos_ik_P0 - obs_centers_matrix(:, j)) + ...
                            2*(Agent_pos_ik_P0 - obs_centers_matrix(:, j))'*(Agent_pos_ik - Agent_pos_ik_P0);
        ...
        function metrics = getBenchmarkingMetrics(obj)
            ...
            % keyboard;
            metrics.formation_constraint_violations = norm(full(obj.formation_constraint_violations));
        end
