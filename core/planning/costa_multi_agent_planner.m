% costa_multi_agent_planner.m
function planned_trajectories = costa_multi_agent_planner(agents, env_params, current_params, sim_params, agent_params)
...
% Formulation: Direct Transcription (positions are decision variables).
    ...
    % [ocean_current_func, ocean_linear_approx_func] = create_symbolic_ocean_func(current_params, true); % Use MX
    ocean_current_func = create_symbolic_ocean_func(current_params, true); % Use MX
    ocean_gradient_func = create_symbolic_ocean_gradient_func(current_params, true); % Use MX
    ...
    for k = 0:T-1 % Loop through time intervals 0 to T-1
        ...
        P0_k = P0(:, k+1); % Reference position for linearization

        % Reshape state vector to 2xN_agents matrix for current function
        Pos_k_matrix = reshape(P_k, 2, N_agents);
        P0_k_matrix = reshape(P0_k, 2, N_agents);

        % Calculate currents at time k*dt for all agents
        % Currents_k_matrix = ocean_linear_approx_func(Pos_k_matrix, P0_k_matrix, k*dt);
        Currents_k_matrix = MX.zeros(2, N_agents);
        Current_at_P0 = ocean_current_func(P0_k_matrix, k*dt);
        jacobians = ocean_gradient_func(P0_k_matrix, k*dt);
        ...
        % Currents_k_matrix = ocean_current_func(P0_k_matrix, k*dt) + ocean_gradient_func(P0_k_matrix, k*dt) * (Pos_k_matrix - P0_k_matrix);
    ...
    for k = 0:T-1 % Loop intervals
        P_k = P_sym(:, k+1); P_k_plus_1 = P_sym(:, k+2); P0_k = P0(:, k+1);
        Pos_k_matrix = reshape(P_k, 2, N_agents); P0_k_matrix = reshape(P0_k, 2, N_agents);
    ...
    try
        sol = solver('x0', P0(:), 'lbx', lbx, 'ubx', ubx, 'lbg', lbg, 'ubg', ubg);
