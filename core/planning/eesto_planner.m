...
function [planned_trajectories, metrics] = eesto_planner(agents, env_params, current_params, sim_params, agent_params)
    ...
    for i = 1:length(agents)
        ...
        t_start = tic;
        [EESTO_path, EESTO_energy,cost_EESTO,V_rel_i,V_abs_i] = EESTO_dylan_mohan(v_max,N,num_its,decay_fact,...
        ...
        planned_trajectories{i} = struct('planned_positions', EESTO_path(:,1:2)');
        P0_final(2*i-1:2*i,:) = EESTO_path(:,1:2)';
        % metrics{i} = struct('EESTO_energy', EESTO_energy, 'cost_EESTO', cost_EESTO, 'V_rel_i', V_rel_i, 'V_abs_i', V_abs_i);
    end
