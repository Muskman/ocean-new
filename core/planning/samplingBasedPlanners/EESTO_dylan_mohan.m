function [EESTO_path, EESTO_energy,cost_EESTO,V_rel_i,V_abs_i] = EESTO_dylan_mohan(v_max,K,num_its,decay_fact,...
                                          ...
                                          x_min,x_max,y_min,y_max,u_true,v_true,opts)
...
while (flag && m <= num_its)
    ...
    u = u_true; % + opts.noise_u_all{m};
    v = v_true; % + opts.noise_v_all{m};
   
    if any(any(isnan(path)))
        keyboard
    end    
    ...
    for i = 1:K-1
        ...
        temp_eps(3,:) = temp_eps(3,:)*10;%*0.1;% *1e6; %* .1;
        temp_eps = temp_eps.';
        ...
        noisy_paths(i,3,:) = noisy_paths(i,3,:)*(sum(path(:,3)) / sum(noisy_paths(i,3,:))); 
        for j = 1:N-1
            temp_eps(j,3) = noisy_paths(i,3,j) - path(j,3);
        end
        eps_mat(i,:,:) = temp_eps.';
    end
    temp_eps = zeros(3,N).';
    ...
    if any(any(isnan(update_vector)))
        keyboard
    end
