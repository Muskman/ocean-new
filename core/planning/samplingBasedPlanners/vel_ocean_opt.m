function [vel_u, vel_v] = vel_ocean_opt(theta_x,theta_y,U,V,X_loc,Y_loc,current_params)
%
    N = size(theta_x,1);
    K = size(theta_y,2);
    ...
    for i = 1:N
        for j = 1:K
            ...
            %     vel_v(i,j) = inf;
            % end
            [vel_u(i,j), vel_v(i,j)] = calculate_ocean_current_vectorized(way_point, 0, current_params);
        end
    end

%    [vel_u, vel_v] = calculate_ocean_current_vectorized([theta_x; theta_y], 0, current_params);
