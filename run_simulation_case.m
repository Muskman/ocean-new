function all_metrics = run_simulation_case(sim_params, env_params, current_params, agent_params, video_params)
    ...
    for algo_idx = 1:length(algorithms_to_run)
        ...
        % Deterministic algorithms run once; stochastic ones run num_mc_simulations times
        deterministic_algos = {'astar', 'fullOpt'};
        is_deterministic = any(strcmp(current_algo, deterministic_algos));
        if is_deterministic
            n_mc = 1;
        else
            n_mc = length(all_metrics.mc_random_seed);
        end

        metrics_runs = cell(n_mc, 1);% repmat(struct('success', false, 'error_message', ''), n_mc, 1);

        for j = 1:n_mc
            local_sim_params = sim_params;
            ...
            local_video_params = video_params;
            if j < n_mc
                local_sim_params.visualization = false;
                local_video_params.enabled = false;
                if isfield(local_video_params, 'save_figure'); local_video_params.save_figure = false; end
            end
            ...
            try
                ...
                % --- Initialization ---
                agents = initialize_agents(num_agents, agent_params, env_params, local_sim_params);
                state_history = cell(num_agents, 1);
                for i = 1:num_agents
                    state_history{i} = nan(2, local_sim_params.time_steps + 1);
                    state_history{i}(:, 1) = agents(i).position;
                    agents(i).current_plan = []; agents(i).plan_start_step = -inf;
                end

                % --- Initialize Visualization ---
                visualizer = SimulationVisualizer(local_sim_params, env_params, current_params, agent_params, num_agents, local_video_params);
                visualizer.initialize(agents);

                fprintf('Starting simulation loop...\n');

                % --- Simulation Loop ---
                for t_idx = 1:local_sim_params.time_steps
                    ...
                    % 2. Plan Trajectories
                    if mod(t_idx - 1, local_sim_params.replan_interval) == 0 || t_idx == 1
                        fprintf('Step %d (t=%.1f): Re-planning...\n', t_idx, current_time);

                        switch local_sim_params.algo
                            case 'fullOpt'
                                [planned_trajectories, metrics] = fullopt_multi_agent_planner(agents, env_params, current_params, local_sim_params, agent_params);
                            case {'sca', 'ssca'}
                                [planned_trajectories, metrics] = sca_multi_agent_planner(agents, env_params, current_params, local_sim_params, agent_params);
                            case 'dssca'
                                [planned_trajectories, metrics] = dssca_multi_agent_planner(agents, env_params, current_params, local_sim_params, agent_params);
                            case 'eesto'
                                [planned_trajectories, metrics] = eesto_planner(agents, env_params, current_params, local_sim_params, agent_params);
                            case 'stomp'
                                [planned_trajectories, metrics] = stomp_planner(agents, env_params, current_params, local_sim_params, agent_params);
                            case 'astar'
                                [planned_trajectories, metrics] = astar_planner(agents, env_params, current_params, local_sim_params, agent_params);
                            otherwise
                                error('Unknown algorithm: %s. Valid options: fullOpt, sca, ssca, dssca, eesto, stomp, astar', local_sim_params.algo);
                        end
                    ...
                    for i = 1:num_agents
                        ...
                        if ~isempty(plan) && time_into_plan < size(plan, 2) - 1
                            ...
                            required_ground_velocity = required_displacement / local_sim_params.dt;
                            required_control_velocity = required_ground_velocity - agents(i).estimated_current;
                            speed = norm(required_control_velocity);
                            if speed > agent_params.max_speed
                                agents(i).control_velocity = required_control_velocity * (agent_params.max_speed / speed);
                            else
                                agents(i).control_velocity = required_control_velocity;
                            end
                        else
                ...
                fprintf('Algorithm %s finished after %.1f seconds.\n', current_algo, local_sim_params.T_final);

                metrics.success       = true;
                metrics.error_message = '';
                metrics.mc_random_seed = local_sim_params.mc_random_seed;
                ...
                warning('run_simulation_case:AlgorithmFailed', ...
                    'Algorithm %s (mc %d/%d) failed: %s', current_algo, j, n_mc, ME.message);
                metrics = struct();
            ...
            metrics_runs{j} = metrics;
        end

        all_metrics.(current_algo) = metrics_runs;
